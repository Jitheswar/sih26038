...
%     'OutputDir'  Where the manifest is written (default ~/sih-demo).
%
...
% in the split, and scenario 8 below is where Level 4 is demonstrated.
scenarios(end + 1) = localScenario( ...
...
scenarios(end + 1) = localScenario( ...
    ...
    0.50, @(d, ~, ~) d.decision == "escalate" & ~d.spatiallyAgree);

scenarios(end + 1) = localScenario( ...
    'Escalate, evidence contradicts CNN', ...
    'The lesion channel does not support what the classifier claims.', ...
    0.50, @(d, ~, ~) d.decision == "escalate" & ~d.evidenceSupportsCNN);

scenarios(end + 1) = localScenario( ...
    'Escalate, insufficient evidence', ...
    'The two channels disagree about referral itself, not about severity.', ...
    0.50, @(d, ~, ~) d.decision == "escalate" & ...
        d.agreementStatus == "insufficient evidence");
...
scenarios(end + 1) = localScenario( ...
    ...
    0.95, @(d, ~, ~) d.decision == "escalate" & d.predictedLevel >= 4);
...
% Requires that A5 also handled this case autonomously, so it is a
% different story from scenario 9 rather than the same repair told twice.
% Without that clause both scenarios select a CNN-0 / rule-1 auto-clear and
% a presenter shows the same mechanism under two labels.
scenarios(end + 1) = localScenario( ...
    'Concordant, severity differs only', ...
    ['Levels differ (CNN 0, rule 1), both below referral, so the case ' ...
    'is handled. Under exact equality this same image escalates: it is ' ...
    'A9 that would have sent it to a human, not A5, which never ran the ' ...
    'comparison at all because the classical channel caps at Level 1.'], ...
    0.20, @(d, p, ~) d.agreementStatus == "concordant" & ...
        d.predictedLevel ~= d.ruleLevel & p.autonomous);

scenarios(end + 1) = localScenario( ...
    'Rule engine reaches Level 2', ...
    'Hard-exudate evidence lifts the rule ceiling off Level 1.', ...
    0.60, @(d, ~, ~) d.ruleLevel >= 2);

scenarios(end + 1) = localScenario( ...
    'Referable, escalated not missed', ...
    'A referable patient the pipeline declined to decide. The safe failure.', ...
    0.50, @(d, ~, s) d.decision == "escalate" & s.grade >= 2);
end
